...
# --- DGP1 (harmonic-decay reference, comparison only) ---
generate_data_v1 <- function(n, p, s = 4, outcome = "linear", misspec = FALSE,
...
# --- equivalence check ---
for (oc in c("linear","quad1","exp")) for (ms in c(FALSE,TRUE)) for (cc in c("iid","ar1")) {
  set.seed(42); a <- generate_data_v1(500, 50, outcome = oc, misspec = ms, covcor = cc, signs = "ks")
  set.seed(42); b <- generate_data   (500, 50, outcome = oc, misspec = ms, covcor = cc, signs = "ks")
  stopifnot(identical(a, b))
}
cat("PASS: all 12 cells identical\n")


source("your_script.R", echo = TRUE)
getwd()
...
grid <- expand.grid(
  ...
  misspec  = c(FALSE, TRUE),
  covcor   = "iid",
  overlap  = c(4,2,1,0.75,0.5,0.25),
  signs    = c("ks"),
  strength = 1,
...
parSapply(cl, 1:num.sim, function(i) {
  ...
  tryCatch({
    ...
    fit_cv <- cv.balnet(X, W, nfolds = 5, max.imbalance = 0.005)
...
write.csv(out.df, gzfile("DGP2OVERLAP+MISSPEC.csv.gz"), row.names = FALSE)
