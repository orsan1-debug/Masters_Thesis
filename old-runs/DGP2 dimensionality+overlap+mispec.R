...
grid <- expand.grid(
  ...
  misspec     = c(TRUE,FALSE),
  covcor      = "iid",
  overlap     = c(1, 0.75, 0.5, 0.25),
  signs       = "pos",
  strength    = 1,
  decay       = 1,
  outcome_set = "fixed4",
...
grid$p <- grid$n * grid$pn_ratio   #fixed p/n as a ration. 
...
run_sim <- function(n, p, s = 4, outcome = "linear", misspec = FALSE, covcor = "iid",
                    ...
                    decay = 1, outcome_set = "fixed4", num.sim = 1000) {
...
write.csv(out.df, gzfile("DGP2dimensionality+mispecc+overlap.csv.gz"), row.names = FALSE)
