...
for (j in seq_len(nrow(cells))) {
  cell <- cells[j, ]
  out_file <- file.path(res_dir, paste0("wc_noise_s", cell$sigma, "_c", cell$c, ".rds"))
...
written <- list.files(res_dir, "^wc_noise_s\\d+_c\\d+\\.rds$", full.names = TRUE)
if (any(file.mtime(written) >= run_start)) {
  register_run(component = "cv", dgp = "dgp_wc_overlap", estimators = c("cv.bloss", "cv.smd", "cv.inf", "boot.smd", "boot.inf", "alg1", "glmnet", "glm", "naive"),
               n_sim = n_rep, seed = master_seed, script = "runs/cv/wz_replication/run_wc_noise.R",
               result_file = file.path(res_dir, "wc_noise_s*_c*.rds"))
}