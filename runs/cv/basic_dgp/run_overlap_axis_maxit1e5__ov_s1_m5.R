...
written <- list.files(res_dir, sprintf("^ov_s1_m5_\\d+_r%d\\.rds$", n_rep), full.names = TRUE)
if (any(file.mtime(written) >= run_start)) {
  register_run(component = "cv", dgp = "gen_data", estimators = c("cv.bloss", "cv.smd", "cv.inf", "boot.smd", "boot.inf"),
               n_sim = n_rep, seed = "2400 + i", script = "runs/cv/basic_dgp/run_overlap_axis_maxit1e5__ov_s1_m5.R",
               result_file = file.path(res_dir, sprintf("ov_s1_m5_*_r%d.rds", n_rep)))
}
