# run_p_x_alpha_x_overlap__dima.R: p {100, 500} x alpha {0.5, 0.75} x overlap {moderate, bad},
# n = 1000, sigma_y = 1, 200 reps. From balnet_mini_experiment_Machine_2.R
# lines 762-834. Writes results/runs/dima_<ii>_r200.rds and
# figures/dim_by_alpha_r200.png.
...
for (i in seq_len(nrow(grid))) {
  out_file <- file.path(res_dir, sprintf("dima_%02d_r%d.rds", i, n_rep))
...
written <- list.files(res_dir, sprintf("^dima_\\d+_r%d\\.rds$", n_rep), full.names = TRUE)
if (any(file.mtime(written) >= run_start)) {
  register_run(component = "cv", dgp = "gen_data", estimators = c("cv.bloss", "cv.smd", "cv.inf", "boot.smd", "boot.inf"),
               n_sim = n_rep, seed = "1900 + i", script = "runs/cv/basic_dgp/run_p_x_alpha_x_overlap__dima.R",
               result_file = file.path(res_dir, sprintf("dima_*_r%d.rds", n_rep)))
}

#  plot: rows overlap, cols alpha within each p block ----
files <- list.files(res_dir, sprintf("^dima_\\d+_r%d\\.rds$", n_rep),
                    full.names = TRUE)
rmse <- function(m) sqrt(colMeans(m^2))          # true value is 0
cols <- c("blue", "blue", "blue", "red", "red")
ltys <- c(4, 1, 2, 1, 2)
png(file.path(fig_dir, sprintf("dim_by_alpha_r%d.png", n_rep)), 2000, 800,
    res = 110)
