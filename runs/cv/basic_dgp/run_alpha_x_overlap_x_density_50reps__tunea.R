...
n_rep <- as.integer(Sys.getenv("N_SIM", "50"))   # smoke test, then 50
...
written <- list.files(res_dir, sprintf("^tunea_\\d+_r%d\\.rds$", n_rep), full.names = TRUE)
if (any(file.mtime(written) >= run_start)) {
  register_run(component = "cv", dgp = "gen_data", estimators = c("cv.bloss", "cv.smd", "cv.inf", "boot.smd", "boot.inf"),
               n_sim = n_rep, seed = "600 + i", script = "runs/cv/basic_dgp/run_alpha_x_overlap_x_density_50reps__tunea.R",
               result_file = file.path(res_dir, sprintf("tunea_*_r%d.rds", n_rep)))
}
