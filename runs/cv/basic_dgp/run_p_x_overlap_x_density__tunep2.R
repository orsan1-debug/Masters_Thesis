...
n_rep <- as.integer(Sys.getenv("N_SIM", "50"))   # smoke test, then 200
...
written <- list.files(res_dir, sprintf("^tunep2_\\d+_r%d\\.rds$", n_rep), full.names = TRUE)
if (any(file.mtime(written) >= run_start)) {
  register_run(component = "cv", dgp = "gen_data", estimators = c("cv.bloss", "cv.smd", "cv.inf", "boot.smd", "boot.inf"),
               n_sim = n_rep, seed = "200 + i", script = "runs/cv/basic_dgp/run_p_x_overlap_x_density__tunep2.R",
               result_file = file.path(res_dir, sprintf("tunep2_*_r%d.rds", n_rep)))
}
