...
written <- list.files(res_dir, sprintf("^alpha_bad_\\d+_r%d\\.rds$", n_rep), full.names = TRUE)
if (any(file.mtime(written) >= run_start)) {
  register_run(component = "cv", dgp = "gen_data", estimators = c("cv.bloss", "cv.smd", "cv.inf", "boot.smd", "boot.inf"),
               n_sim = n_rep, seed = "1200 + i", script = "runs/cv/basic_dgp/run_alpha_sweep_bad_overlap__alpha_bad.R",
               result_file = file.path(res_dir, sprintf("alpha_bad_*_r%d.rds", n_rep)))
}
