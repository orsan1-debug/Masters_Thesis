...
written <- list.files(res_dir, sprintf("^tunea3_\\d+_r%d\\.rds$", n_rep), full.names = TRUE)
if (any(file.mtime(written) >= run_start)) {
  register_run(component = "cv", dgp = "gen_data", estimators = c("cv.bloss", "cv.smd", "cv.inf", "boot.smd", "boot.inf"),
               n_sim = n_rep, seed = "800 + i", script = "runs/cv/basic_dgp/run_ridge_enet_x_overlap_x_density__tunea3.R",
               result_file = file.path(res_dir, sprintf("tunea3_*_r%d.rds", n_rep)))
}
