...
written <- list.files(res_dir, sprintf("^tune2_\\d+_r%d\\.rds$", n_rep), full.names = TRUE)
if (any(file.mtime(written) >= run_start)) {
  register_run(component = "cv", dgp = "gen_data", estimators = c("cv.bloss", "cv.smd", "cv.inf", "boot.smd", "boot.inf"),
               n_sim = n_rep, seed = "500 + i", script = "runs/cv/basic_dgp/run_noise_x_overlap_density_fixed__tune2.R",
               result_file = file.path(res_dir, sprintf("tune2_*_r%d.rds", n_rep)))
}
