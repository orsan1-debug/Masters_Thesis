# summarise_runs_to_csv.R: cleaned summary tables for the CV exploration.
# Reads results/runs/*_r<n>.rds, writes results/summary/cv_summary.csv,
# cv_family_counts.csv, cv_vs_lasso_floor.csv (read by cv_summary.qmd).
rm(list = ls())
library(dplyr)
# *** Setup ***
...
stopifnot(dir.exists(res_dir), dir.exists(sum_dir))
...
fams <- sub("_?\\d+_r\\d+\\.rds$", "", list.files(res_dir, "_r\\d+\\.rds$"))
...
files <- list.files(res_dir, "_r\\d+\\.rds$", full.names = TRUE)
