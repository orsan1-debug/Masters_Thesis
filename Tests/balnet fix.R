...
## seeds identical to broken base_seed 202 batches: fixed-lambda + glmnet cols
## must match those batches exactly; balnetcv differs only via lambda selection
source("R/dgp.R"); source("R/estimators.R"); source("R/simulate.R"); source("R/registry.R")
...
balnet_sha <- tryCatch(utils::packageDescription("balnet")$RemoteSha, error = function(e) NA_character_)
