...
run_sim <- function(n, p, s = 4, outcome = "linear", misspec = FALSE, covcor = "iid",
  ...
  parSapply(cl, 1:num.sim, function(i) {
    ...
    tryCatch({
      data <- generate_data(n = n, p = p, s = s, outcome = outcome,
                            misspec = misspec, covcor = covcor, overlap = overlap,
...
write.csv(out.df, gzfile("DGP2corsignTEST.csv.gz"), row.names = FALSE)
