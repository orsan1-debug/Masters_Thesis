...
![](fig_picks_vs_opt.png){#fig-picks-vs-opt alt="Median selected lambda for cv.bloss and boot.inf against the RMSE-optimal lambda, along the noise axis (good overlap) and the overlap axis ($\\sigma_y = 1$). Log scales."}

![Median selected lambda for cv.bloss and boot.inf against the RMSE-optimal lambda, by overlap level. Log scales.](fig_picks_grid.png){#fig-picks-grid}
...
![](fig_alpha_sweep.png){width="100%" fig-pos="H"}
...
![](fig_ridge_floor.png){width="100%" fig-pos="H"}

**Figure:** ridge (alpha 0), fixed overlap,

## Ridge against the 1e-4 floor

![](fig_enet_floor.png){width="100%" fig-pos="H"}
